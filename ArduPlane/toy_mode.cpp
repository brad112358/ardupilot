...
const AP_Param::GroupInfo ToyMode::var_info[] = {
    ...
    // @Description: Bitmask of flags to change the behaviour of tmode. DisarmOnLowThrottle means to disarm if throttle is held down for 1 second when landed. ArmOnHighThrottle means to arm if throttle is above 80% for 1 second. UpgradeToLoiter means to allow takeoff in LOITER mode by switching to CIRCLE, then auto-upgrading to LOITER once GPS is available. RTLStickCancel means that on large stick inputs in RTL mode that LOITER mode is engaged
...
void ToyMode::update()
{
    ...
    if (!done_first_update) {
        ...
        plane.set_mode(control_mode_t(primary_mode[0].get()), MODE_REASON_TMODE);
    }
    ...
    // set CIRCLE as indoors for the EKF (disables GPS vertical velocity fusion)
#if 0
    plane.ahrs.set_indoor_mode(plane.control_mode == CIRCLE || plane.control_mode == FLOWHOLD);
#endif
    ...
    if (plane.failsafe.rc_failsafe || ch5_in < 900) {
    ...
    if (reset_combination && abs(plane.ahrs.roll_sensor) > 160) {
        /*
    ...
    bool throttle_near_max =
        plane.channel_throttle->get_control_in() > 700;
    
    /*
      arm if throttle is high for 1 second when landed
     */
    if ((flags & FLAG_THR_ARM) && throttle_near_max && !plane.arming.is_armed()) {
        throttle_high_counter++;
        if (throttle_high_counter >= TOY_LAND_ARM_COUNT) {
            gcs().send_text(MAV_SEVERITY_INFO, "Tmode: throttle arm");
            arm_check_compass();
            if (!plane.arm_motors(AP_Arming::ArmingMethod::MAVLINK) && (flags & FLAG_UPGRADE_LOITER) && plane.control_mode == LOITER) {
                /*
                  support auto-switching to CIRCLE, then upgrade to LOITER once GPS available
                 */
                if (set_and_remember_mode(CIRCLE, MODE_REASON_TMODE)) {
                    gcs().send_text(MAV_SEVERITY_INFO, "Tmode: CIRCLE update arm");
#if GEOFENCE_ENABLED == ENABLED
                    plane.geofence_set_enabled(false, AUTO_TOGGLED);
#endif
                    if (!plane.arm_motors(AP_Arming::ArmingMethod::MAVLINK)) {
                        // go back to LOITER
                        gcs().send_text(MAV_SEVERITY_ERROR, "Tmode: CIRCLE arm failed");
    ...
    if (upgrade_to_loiter) {
        if (!plane.arming.is_armed() || plane.control_mode != CIRCLE) {
            upgrade_to_loiter = false;
#if 0
            AP_Notify::flags.hybrid_loiter = false;
#endif
        } else if (plane.have_position && set_and_remember_mode(LOITER, MODE_REASON_TMODE)) {
#if GEOFENCE_ENABLED == ENABLED
plane.geofence_set_enabled(true, AUTO_TOGGLED);
#endif
            gcs().send_text(MAV_SEVERITY_INFO, "Tmode: LOITER update");            
        }
    }

    if (plane.control_mode == RTL && (flags & FLAG_RTL_CANCEL) && throttle_near_max) {
        gcs().send_text(MAV_SEVERITY_INFO, "Tmode: RTL cancel");        
        set_and_remember_mode(LOITER, MODE_REASON_TMODE);
    }
    
    control_mode_t old_mode = plane.control_mode;
    control_mode_t new_mode = old_mode;
    ...
    if (new_mode != plane.control_mode) {
        load_test.running = false;
#if GEOFENCE_ENABLED == ENABLED
plane.geofence_set_enabled(false, AUTO_TOGGLED);
#endif
        if (set_and_remember_mode(new_mode, MODE_REASON_TX_COMMAND)) {
            // gcs().send_text(MAV_SEVERITY_INFO, "Tmode: mode %s", plane.flightmode->name4());
            ...
            if (new_mode == RTL) {
                ...
                set_and_remember_mode(QLAND, MODE_REASON_TMODE);
            }
        }
    }
}
...
#if 0
    if (!plane.set_mode(mode, reason)) {
        return false;
    }
#endif
    plane.set_mode(mode, reason);
    last_set_mode = mode;
    return true;
}
...
{
    if (hal.util->get_soft_armed() || plane.failsafe.rc_failsafe) {
    ...
    uint16_t throttle_trim = plane.channel_throttle->get_radio_trim();
    if (abs(throttle_trim - 1500) <= trim_auto) {
        RC_Channel *c = plane.channel_throttle;
    ...
    uint16_t chan[4];
    if (RC_Channels::get_singleton()->get_radio_in(chan, 4) != 4) {
        trim.start_ms = 0;
        return;
    }
...
void ToyMode::action_arm(void)
{
    ...
    if (needs_gps && plane.arming.gps_checks(false)) {
#if GEOFENCE_ENABLED == ENABLED
        // we want GPS and checks are passing, arm and enable fence
        plane.geofence_set_enabled(true, AUTO_TOGGLED);
#endif
        plane.arm_motors(AP_Arming::ArmingMethod::RUDDER);
        if (!plane.arming.is_armed()) {
...
void ToyMode::blink_update(void)
{
    ...
    if (plane.arming.is_armed() && AP_Notify::flags.failsafe_battery) {
        pattern = BLINK_8;
    } else if (!plane.arming.is_armed() && (blink_disarm > 0)) {
		pattern = BLINK_8;
		blink_disarm--;
	} else {
        pattern = BLINK_FULL;
...
void ToyMode::arm_check_compass(void)
{
    ...
    Vector3f offsets = plane.compass.get_offsets();
    float field = plane.compass.get_field().length();
    
    if (offsets.length() > plane.compass.get_offsets_max() ||
        field < 200 || field > 800 ||
        !plane.compass.configured()) {
        if (plane.compass.get_learn_type() != Compass::LEARN_INFLIGHT) {
            gcs().send_text(MAV_SEVERITY_INFO, "Tmode: enable compass learning");
            plane.compass.set_learn_type(Compass::LEARN_INFLIGHT, false);
        }
    }
}
